(function(window){
	...
	Guaiwulieren.prototype = {
		...
		printdialogcard:function(id){//打印武器属性dialog
			...
			//弩炮和弓没有斩味属性
			if(that.getsession("wbgl-guaiwulieren-weapon-type") != 6 && that.getsession("wbgl-guaiwulieren-weapon-type") != 7){
			...
			if(that.getsession("wbgl-guaiwulieren-weapon-type") == 6){
				part2 = part2 + '<tr>\
							<td><em>装填速度：</em></td>\
							<td><em>'+data[27]+'</em></td>\
						</tr>';
			...
			}else if(that.getsession("wbgl-guaiwulieren-weapon-type") == 7){
				...
				if(that.getsession("wbgl-guaiwulieren-weapon-type") == 8)//狩猎笛
				{
				...
				if(that.getsession("wbgl-guaiwulieren-weapon-type")==0)//铳枪炮击属性
				{
				...
				if(data[90]!='')//锻造材料
				{
					...
					for(var j = 0;j<16;j=j+3)
					{
						...
						{
							part3 = part3 + '<tr>\
										<td><em>'+data[94+j]+data[94+j]+'×'+data[95+j]+'<em></td>\
									</tr>';
						}
						else
						{
				...
				if(data[113]!='')//派生材料
				{
					...
					for(var j = 0;j<16;j=j+3)
					{
						...
						{
							part3 = part3 + '<tr>\
										<td><em>'+data[117+j]+data[117+j]+'×'+data[116+j]+'</em></td>\
									</tr>';
							k = 2;
						}
						else

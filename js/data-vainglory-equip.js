(function(window){
	var QmcsEquip = function(option){
		...
		this.o = {platform:"web",plugin:"plugin_1103",plugin_hero:"plugin_1101",url:"images/vainglory/"};
	...
	QmcsEquip.prototype = {
		htmlDetail: function(){//详情页
			var that = this,
				...
				id = Number(that.getsession("wbgl-vainglory-equip-id")),//装备id
				piece,
				html_equipd_head = '',
			...
			html_equipd_head = '<div class="l"><img src="'+that.o.url + 'equip/item' + id +'.png"></div>'+
								'<div class="r">'+
			...
			for(var i=0; i<piece.need.length; i++){
				html_equipd_need_ul += '<li data-id="'+piece.need[i]+'"><img src="'+that.o.url + 'equip/item' + piece.need[i] +'.png"></li>';
			}
			...
			$("#equipd_need_ul").on("click", "li", function(){//点击合成所需跳转
				...
				that.setsession("wbgl-vainglory-equip-id", _id);
			...
			$("#equipd_fithero").on("click", "li", function(){//适合英雄跳转
				...
				if(that.o.platform == "ios"){
					that.setsession("wbgl-vainglory-equip2hero", "y");//保证跳转到英雄详情页，然后再跳回来
					location.href = that.o.plugin_hero+'/data-vainglory-hero-detail.html';
		...
		htmlIndex: function(){//列表页
			var html_equip_head = '',
				...
				piece,
				t_index = Number(that.getsession("wbgl-vainglory-equip-tindex"));			
			...
			$("#equip_content").on("click", "li", function(){
				...
				that.setsession("wbgl-vainglory-equip-tindex", t_index);
				that.setsession("wbgl-vainglory-equip-id", _id);
